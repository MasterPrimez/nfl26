...
export function renderField(g, sum) {
  ...
  const play = plays[plays.length - 1] || null;
  const sit = sum?.header?.competitions?.[0]?.situation || null;
  ...
  const spot = node => node ? (uFromYTE(node.yardsToEndzone) ?? uFromText(node.possessionText || node.text)) : null;
  let u1 = spot(play?.end) ?? uFromText(sit?.possessionText) ?? null;
  ...
  const text = play?.text || g.situation?.lastPlay || '';
  ...
  let p1 = '', p2 = '';
  if (kind === 'pass') { const m = /^(.+?) pass(?:.*?) to (.+?)(?: for| \(|,|\.$|$)/.exec(text); if (m) { p1 = m[1]; p2 = m[2]; } else { const s = /^(.+?) (?:pass|sacked)/.exec(text); if (s) p1 = s[1]; } }
  else if (kind === 'run') { const m = /^(.+?) (?:run|rush|scramble|left|right|up the middle)/i.exec(text); if (m) p1 = m[1]; }
  else { const m = /^(.+?) (?:punt|kick|field goal)/i.exec(text); if (m) p1 = m[1]; }
  ...
  const cfg = { u0, u1, uFD, dir, kind, incomplete, scoring, p1: abbrevName(p1).toUpperCase(), p2: abbrevName(p2).toUpperCase(), animate, off: { color: off.color, logo: off.logo }, redZone: dir > 0 ? [90, 110] : [10, 30], away: { name: g.away.name, color: g.away.color, logo: g.away.logo }, home: { name: g.home.name, color: g.home.color, logo: g.home.logo } };
...
export function mountField(root) {
  ...
  const pin = (u, label, o) => { const [x0, y0] = P(u, v, 0), [x1, y1] = P(u, v, 7); return `<g opacity="${o}"><ellipse cx="${x0}" cy="${y0}" rx="9" ry="4" fill="rgba(0,0,0,.5)"/><line x1="${x0}" y1="${y0}" x2="${x1}" y2="${y1}" stroke="#fff" stroke-width="2"/><circle cx="${x1}" cy="${y1 - 20}" r="22" fill="#fff" stroke="${c.off.color}" stroke-width="3"/><image href="${esc(c.off.logo)}" x="${x1 - 16}" y="${y1 - 36}" width="32" height="32"/>${label ? `<rect x="${x1 - 34}" y="${y1 + 4}" width="68" height="14" rx="3" fill="rgba(0,0,0,.7)"/><text x="${x1}" y="${y1 + 14}" font-family="IBM Plex Mono,monospace" font-size="9" fill="#fff" text-anchor="middle" letter-spacing="1">${esc(label)}</text>` : ''}</g>`; };
  ...
  const draw = (e, after) => {
    ...
    d += pin(u0, c.p1, 1);
    if (e >= 1 && c.p2 && !c.incomplete) d += pin(u1, c.p2, Math.min(1, after / 300));
    const [sx, sy] = P(u, v, 0);
